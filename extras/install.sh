...
# Quickshell home dashboard (the only part driftwm itself doesn't provide).
backup quickshell
cp -r -- "$src/quickshell" "$cfg/quickshell"

# Shader wallpapers, so [background] can point at a stable path.
backup wallpapers
cp -r -- "$src/wallpapers" "$cfg/wallpapers"
...
missing=""
for tool in qs nmcli swaync swayosd-server fuzzel jq swaylock swayidle \
            brightnessctl playerctl wl-copy notify-send; do
