...
- **`MultiRenderer: Offscreen<GlesTexture> + Bind<GlesTexture>`**, but both route to the TARGET device — fine when render == target (`single_renderer`), wrong GPU for a true cross-GPU renderer. `as_mut()` (driftwm's `as_gles_renderer()`) returns the RENDER device's GlesRenderer, so never mix the two on one texture.
- **`MultiRenderer: ExportMem`** routes per variant: `copy_framebuffer` on a `MultiFramebuffer::Target` copies via the target device, and `map_texture` maps the resulting `MultiTextureMapping` on the device that produced it — so bind → render → `copy_framebuffer` → `map_texture` all through the same `MultiRenderer` is cross-GPU-safe.
